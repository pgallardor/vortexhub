...
create or replace function public.set_store_public_visibility(store_id uuid, is_visible boolean)
...
begin
  if not public.is_store_wide_manager(store_id) then
    raise exception 'Store-wide manager permission required' using errcode = '42501';
  end if;

  update public.stores
  set is_publicly_visible = is_visible
  where id = store_id
    and status = 'active'
  ...
  if result.id is null then
    raise exception 'Active store not found' using errcode = 'P0002';
  end if;

  perform public.audit(
    case when is_visible then 'store.public_visibility.enabled' else 'store.public_visibility.disabled' end,
    'store',
    result.id,
    result.id,
    null,
    jsonb_build_object('isPubliclyVisible', is_visible)
  );
...
as $$
  with filtered as (
    ...
    where e.status = 'published' and e.archived_at is null and e.deleted_at is null
      and s.status = 'active' and s.is_publicly_visible and s.deleted_at is null
...
revoke all on function public.set_store_public_visibility(uuid, boolean) from public;
grant execute on function public.set_store_public_visibility(uuid, boolean) to authenticated;
